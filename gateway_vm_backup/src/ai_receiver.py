#!/usr/bin/env python3
"""
ai_receiver.py -- AI Defect Detection Receiver (Phase 5)

Features:
  1. TCP listen on port 8899, receive gateway video frames + MES events
  2. TLV protocol parser + frame reassembly (FRAME_START -> FRAME_DATA)
  3. YUYV -> BGR conversion
  4. YOLO defect detection
  5. AI_RESULT sent back to gateway -> triggers peripherals
...
def crc8(data: bytes) -> int:
    """XOR-based CRC (compatible with C tlv_checksum)"""
    crc = 0
    for byte in data:
        crc ^= byte
    return crc
...
# ========== TLV Stream Parser ==========
class TLVReader:
    """Streaming TLV parser (compatible with C tlv_reader_t)"""
    HEADER = 0
    BODY   = 1
    CRC    = 2
    ...
    def feed(self, data: bytes):
        """Feed byte stream, returns (tag, value) or None"""
        for byte in data:
...
class FrameReassembler:
    """Reassemble FRAME_START -> FRAME_DATA chunks"""
    def __init__(self):
...
class YOLODetector:
    ...
    def __init__(self, model_path: str = None, conf_threshold: float = 0.5):
        ...
        if model_path and os.path.exists(model_path):
            ...
            print("[AI] Place your .pt file at models/yolov11n.pt")
    ...
    def _load_model(self, path: str):
        try:
            ...
            print("[AI] ultralytics not installed, try OpenCV DNN")
            try:
                import cv2
    ...
    def detect(self, yuyv_frame: bytes, width: int, height: int):
        """YUYV frame -> YOLO inference -> results list"""
        import numpy as np
        import cv2
    ...
    def _dummy_detect(self, bgr):
        """Dummy detection: 30% chance of scratch using edge heuristic"""
        import random
        import numpy as np
        import cv2
...
def handle_client(conn, addr, detector, frame_width, frame_height, save_dir=None):
