/*
 * gpio_ops.c -- GPIO control implementation
 */
#include "gpio_ops.h"
#include <stdio.h>
#include <stdlib.h>
...
void gpio_export_all(void) {
    int gpios[] = {GPIO_RELAY, GPIO_BUZZER, GPIO_LED_R,
                   GPIO_LED_G, GPIO_LED_B, GPIO_IR};
    for (int i = 0; i < 6; i++) {
        int fd = open(GPIO_BASE "/export", O_WRONLY);
        if (fd >= 0) {
            char s[4];
            int n = snprintf(s, sizeof(s), "%d", gpios[i]);
            write(fd, s, n);
            close(fd);
        }
    }
...
void rgb_led(const char *color) {
    if (!strcmp(color, "red")) {
        gpio_write(GPIO_LED_R, 1); gpio_write(GPIO_LED_G, 0);
        gpio_write(GPIO_LED_B, 0);
    } else if (!strcmp(color, "green")) {
        gpio_write(GPIO_LED_R, 0); gpio_write(GPIO_LED_G, 1);
        gpio_write(GPIO_LED_B, 0);
    } else if (!strcmp(color, "blue")) {
        gpio_write(GPIO_LED_R, 0); gpio_write(GPIO_LED_G, 0);
        gpio_write(GPIO_LED_B, 1);
    } else if (!strcmp(color, "off")) {
        gpio_write(GPIO_LED_R, 0); gpio_write(GPIO_LED_G, 0);
        gpio_write(GPIO_LED_B, 0);
    } else if (!strcmp(color, "white")) {
        gpio_write(GPIO_LED_R, 1); gpio_write(GPIO_LED_G, 1);
        gpio_write(GPIO_LED_B, 1);
    }
    printf("[GPIO] LED: %s\n", color);
}
